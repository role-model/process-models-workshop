...
* Get the simulations and do some set-up
...
simulated datasets, just to get the hang of it.
...
Cyprus. Fetch the empirical data from Noguerales et al 2021:
...
help to take a quick look at what the dat for this sample looks like, so also
...
exercise we provide the data already collapsed into summary statistics, so all the
imformation we have is number of local species (`local_S`) and the first three
...
traits, and genetic data, so before we can call predict community assembly model
with the real data we need to *re-fit* the rf classifier so it's explanatory
