...
* **Get the simulations:** Simulation results are composed of 'parameters' and 'data'.
Parameters are what goes in to the simulation model and data is what comes out.
* **Transform the simulations:** In some cases parameters/data must be reshaped in
some way to make it more appropriate for the ML approach.
* **Split the data into training/testing sets:** If you train a machine learning model
with **all** of your data, then you won't have any way to really evaluate its
performance. It's very common to split data into 'training' and 'testing' sets, where
the training data used to train the model and the testing data is used to evaluate
...
a **flavor** of how this works, and to show that even when done 'quick and dirty' it
still works pretty okay.
* **Train the model on the training set:** Learn a mapping between patterns in the
data and community assembly model.
* **Test the model on the testing set:** How well does the trained model recover the
'known' assembly model in simulations that it hasn't seen yet.
* **Evaluate variable importance:** Which axes of data contribute most to distinguishing
assembly models?
* **Predict assembly model of empirical data:** And embrace the uncertainty in the
